...
Page({
  ...
  // 点击 切换 tab 栏
  handletabsItemChange(e) {    // mark: handletabsItemChange
      console.log(e);
      let index = e.detail.index;
      let {tabs, goodsList} = this.data;  
    //   console.log(tabs);console.log(goodsList)
      tabs.forEach((item)=>{
  ...
  //监听用户下拉触底事件 加载下一页数据
  onReachBottom() {
    if(this.queryParams.pagenum==this.totalPages || this.queryParams.pagenum>this.totalPages){
        wx.showToast({
            title: '没有下一页数据了',
        });
  ...
  onPullDownRefresh() {
      ...
      // 重新发起请求
      this.getGoodsList()
      // 关闭下拉刷新的效果，若不关闭，加载图标会长时间显示，影响效果
